...
class Database extends Base_1.Base {
    ...
    fetch(query) {
        return new Promise(async (resolve) => {
            if (!['object', 'string'].includes(typeof query))
                return resolve(null);
    ...
    set(key, value) {
        return new Promise(async (resolve) => {
            if (typeof key !== 'string')
                throw new Error('Key should be type string, received:' + typeof key);
    ...
    push(key, value) {
        return new Promise(async (resolve) => {
            ...
            if (!Array.isArray(data))
                throw new Error(`Expected target type to be Array, received ${typeof data}!`);
            if (Array.isArray(value))
                return resolve(await this.set(key, data.concat(value)));
            data.push(value);
    ...
    has(id) {
        return new Promise(async (resolve) => {
            if (typeof id !== 'string')
                throw new Error('ID should be type string, received:' + typeof id);
    ...
    pull(key, value, multiple = false) {
        return new Promise(async (resolve) => {
            ...
            if (typeof multiple !== 'boolean')
                throw new Error('Multiple should be type boolean, received:' + typeof multiple);
            let data = await this.get(key);
            if (!data)
                return resolve(null);
            if (!Array.isArray(data))
                throw new Error(`Expected existing data to be an Array, received ${typeof data}`);
            if (Array.isArray(value)) {
    ...
    get(key) {
        return new Promise(async (resolve) => {
            ...
            let item;
            if (parsed.target)
                item = Data_1.DataUtil.getData(key, Object.assign({}, get.data));
            else
                item = get.data;
    ...
    __math(key, operator, value) {
        return new Promise(async (resolve) => {
            if (typeof key !== 'string')
                throw new Error('Key should be string, received: ' + typeof key);
                ...
                case '+':
                    {
                        ...
                        if (!add) {
                            ...
                            if (typeof add !== 'number')
                                throw new Error(`Expected existing data to be a number, received ${typeof add}`);
                ...
                case '-':
                    {
                        ...
                        if (!less) {
                            ...
                            if (typeof less !== 'number')
                                throw new Error(`Expected existing data to be a number, received ${typeof less}`);
                ...
                case '/':
                    {
                        ...
                        if (!div) {
                            ...
                            if (typeof div !== 'number')
                                throw new Error(`Expected existing data to be a number, received ${typeof div}`);
